...
export const calculateFrameTiming = (
  ...
  const startFrame =
    (startMs * FPS) / 1000 +
    (addIntroOffset ? INTRO_DURATION : 0);

  const duration =
    ((endMs - startMs) * FPS) / 1000 +
    (includeIntro ? INTRO_DURATION : 0);
...
export const calculateBlur = ({
  ...
  if (
    exitTransition === "blur" &&
    localMs > endMs - startMs - fadeMs
  ) {
    return (
      (1 -
        (endMs - startMs - localMs) / fadeMs) *
      maxBlur
    );
  }
